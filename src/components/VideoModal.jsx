...
import { FaTimes, FaHeart, FaComment, FaUserPlus, FaUserCheck, FaPlay, FaInfoCircle } from 'react-icons/fa';
...
const VideoModal = ({ movie, videoId, title, onClose }) => {
    const [view, setView] = useState(movie?.video_url ? 'player' : 'player'); // Default to player for immediate watch experience
    ...
    return (
        <div className="videoModal" onClick={onClose}>
            <div className="videoModal__content flex flex-col" onClick={(e) => e.stopPropagation()}>
                {/* Modal Header Bar with View Switcher */}
                <div className="bg-black/90 px-4 py-2.5 border-b border-white/10 flex items-center justify-between z-50 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        >
                            <FaPlay className="text-[10px]" /> Video Player
                        </button>
                        <button
                        ...
                        >
                            <FaInfoCircle className="text-xs" /> Details & Reviews
                        </button>
                    </div>

                    <button type="button" className="text-white/60 hover:text-white p-2 transition-colors" onClick={onClose} aria-label="Close">
